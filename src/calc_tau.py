...
-Functions:
    ...
    * spect -- spectrogram that returns tx1 frequencies, tx1 times,
    tx1 spectrum
...
def filter_b(b_mfa, ftype='high', comp=2, fs=10, N=1, fc=0.001):
    ...
    input: b_mfa: magnetic field measurements in MFA coordinates, assumed to
           ...
           comp: component of magnetic field to filter 0=radial, 1=phi,
           2=paralell
    ...
    if ftype == 'bandpass'
        ...
        w2 = fs[1] / (fs / 2)
        b, a = u.butter_filter(fs, np.asarray([w1, w2]), N,
                               ftype)  # bandpass filter
...
def get_f_band(f_hp_spectrum, f_low, f_high):
    ...
    band = [f_low,
            f_high]  # band = frequency band limits,: 1-10mHz, use 0.011 so
    # includes 10mHz
...
def get_tau(b_mfa, time, fband, ftype,
            comp):  # function to be called by main module
    """
    calculate tau.
    ...
    output: tau: timescale to diffuse one Lshell, int [min]
            ...
            Sxx_hp_spectrum: power spectrum corresponding to frequency
            domain [nT^2/Hz]
